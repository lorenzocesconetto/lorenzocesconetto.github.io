...
> This blog post got feartured on [Towards Data Science](https://towardsdatascience.com/rope-clearly-explained/)!
...
def rope_angle_between(q, k, d, base=10000.0):
    ...
    dot = float(np.dot(k, q_rot))
    na = float(np.linalg.norm(k))
    nb = float(np.linalg.norm(q_rot))
    cosine = dot / (na * nb)
    return cosine
